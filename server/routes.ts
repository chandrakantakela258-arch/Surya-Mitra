...
export async function registerRoutes(
  ...
  app.get("/api/admin/expenses", requireAdmin, async (req, res) => {
    try {
      const expenses = await storage.getAdminExpenses();
      res.json(expenses);
    } catch (error) {
  ...
  // Get single expense
  app.get("/api/admin/expenses/:id", requireAdmin, async (req, res) => {
    try {
      const expense = await storage.getAdminExpense(req.params.id);
      if (!expense) {
        return res.status(404).json({ message: "Expense not found" });
      }
      res.json(expense);
    } catch (error) {
      console.error("Get admin expense error:", error);
  ...
  app.post("/api/admin/expenses", requireAdmin, async (req, res) => {
    try {
      const userId = (req.user as any)?.id;
      const expense = await storage.createAdminExpense({
        ...req.body,
        createdBy: userId,
      });
      res.status(201).json(expense);
  ...
  app.patch("/api/admin/expenses/:id", requireAdmin, async (req, res) => {
    try {
      const expense = await storage.updateAdminExpense(req.params.id, req.body);
      if (!expense) {
        return res.status(404).json({ message: "Expense not found" });
      }
      res.json(expense);
