...
// Admin Expenses - Mining and operational cost tracking
export const adminExpenses = pgTable("admin_expenses", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  
  // Expense Date and Period
  expenseDate: timestamp("expense_date").notNull().defaultNow(),
  month: text("month").notNull(), // e.g., "January 2026"
  
  // Per Ton Rate Categories
  blastingRatePerTon: decimal("blasting_rate_per_ton", { precision: 10, scale: 2 }),
  quarryOwnerRatePerTon: decimal("quarry_owner_rate_per_ton", { precision: 10, scale: 2 }),
  transportationRatePerTonPerKm: decimal("transportation_rate_per_ton_per_km", { precision: 10, scale: 2 }),
  transportDistanceKm: decimal("transport_distance_km", { precision: 10, scale: 2 }),
  crushingCostPerTon: decimal("crushing_cost_per_ton", { precision: 10, scale: 2 }),
  materialShiftingCostPerTon: decimal("material_shifting_cost_per_ton", { precision: 10, scale: 2 }),
  loadingCostPerTon: decimal("loading_cost_per_ton", { precision: 10, scale: 2 }),
  miningChallanCostPerTon: decimal("mining_challan_cost_per_ton", { precision: 10, scale: 2 }),
  
  // Quantity (tons processed)
  quantityInTons: decimal("quantity_in_tons", { precision: 10, scale: 2 }),
  
  // Fixed/Recurring Costs
  travellingCost: decimal("travelling_cost", { precision: 10, scale: 2 }),
  staffSalary: decimal("staff_salary", { precision: 10, scale: 2 }),
  roomRent: decimal("room_rent", { precision: 10, scale: 2 }),
  departmentCost: decimal("department_cost", { precision: 10, scale: 2 }),
  miscellaneousCost: decimal("miscellaneous_cost", { precision: 10, scale: 2 }),
  miscellaneousDescription: text("miscellaneous_description"),
  
  // Computed total (stored for quick access)
  totalExpense: decimal("total_expense", { precision: 12, scale: 2 }),
...
export const insertAdminExpenseSchema = createInsertSchema(adminExpenses).omit({
  id: true,
  totalExpense: true,
  createdAt: true,
  updatedAt: true,
  ...
  blastingRatePerTon: z.string().or(z.number()).optional().nullable(),
  quarryOwnerRatePerTon: z.string().or(z.number()).optional().nullable(),
  transportationRatePerTonPerKm: z.string().or(z.number()).optional().nullable(),
  transportDistanceKm: z.string().or(z.number()).optional().nullable(),
  crushingCostPerTon: z.string().or(z.number()).optional().nullable(),
  materialShiftingCostPerTon: z.string().or(z.number()).optional().nullable(),
  loadingCostPerTon: z.string().or(z.number()).optional().nullable(),
  miningChallanCostPerTon: z.string().or(z.number()).optional().nullable(),
  quantityInTons: z.string().or(z.number()).optional().nullable(),
  travellingCost: z.string().or(z.number()).optional().nullable(),
  staffSalary: z.string().or(z.number()).optional().nullable(),
  roomRent: z.string().or(z.number()).optional().nullable(),
  departmentCost: z.string().or(z.number()).optional().nullable(),
  miscellaneousCost: z.string().or(z.number()).optional().nullable(),
});
