...
- **Chatbot Admin Panel & Shareable Bot**: Admin can manage all chatbot flow nodes (edit messages, add/remove options, attach media like videos/PPTs/PDFs/images per step) from `/admin/chatbot-settings`. A standalone public page at `/solar-bot` serves as a shareable link — anyone opening it interacts with the chatbot and their data is captured in Solar Bot Leads. The "Share Link" button in the WhatsApp widget copies this `/solar-bot` URL. Data stored in `chatbot_nodes` table. API: `GET /api/public/chatbot-nodes`, `GET/POST/PUT/DELETE /api/admin/chatbot-nodes`.
